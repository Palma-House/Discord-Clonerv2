...
const main = async () => {
  let originServerId = await input.text("ENTER SERVER ID FOR CLONE | BY PALMA ;");
  const createdServerId = await createServer(originServerId);
  console.log("SERVER GG :D");
  ...
  for (let group of parentsAndChannels) {
    if (group.length > 1) {
      ...
      for (let [index, item] of group.entries()) {
        ...
        console.log(`CHANNEL DONE :D ${item.name}`);
      }
    } else {
      await createChannel(
        createdServerId,
      ...
      console.log(`CREATED GROUP :D ${group.name}`);
    }
